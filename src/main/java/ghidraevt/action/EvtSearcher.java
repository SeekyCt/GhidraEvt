...
public class EvtSearcher implements FindDialogSearcher {
	...
	private EvtSearchResults doSearch(String searchText, boolean isRegex) {
		...
		EvtSearchLocation searchLocation = findNext(forwardMatcher, searchText, start);
...
//=================================================================================================	
	...
	public SearchResults search(String text, CursorPosition position, boolean searchForward,
		...
		EvtCursorPosition cursorPosition = (EvtCursorPosition) position;
		FieldLocation startLocation = getNextSearchStartLocation(cursorPosition, searchForward);
		EvtSearchLocation location =
	...
	private FieldLocation getNextSearchStartLocation(
			EvtCursorPosition EvtCursorPosition, boolean searchForward) {

		FieldLocation cursor = EvtCursorPosition.getFieldLocation();
		EvtSearchLocation containingLocation =
	...
	private EvtSearchLocation findNext(Function<String, SearchMatch> matcher,
		...
		for (int i = line; i < fields.size(); i++) {
			...
			return new EvtSearchLocation(fieldLocation, match.start, match.end - 1,
	...
	private String substring(EvtTextField textField, FieldLocation location,
	...
	private FieldLineLocation getFieldIndexFromOffset(int screenOffset, EvtTextField textField) {
