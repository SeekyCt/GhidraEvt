 ...
 * 
 * <p>
 * To add a margin to the disassembler, a client must implement this interface to provide the
 * component that is actually added to the UI. For a reference implementation, see
 * {@link LineNumberEvtMarginProvider}.
 */
public interface EvtMarginProvider {

	/**
	 * Called whenever the program, script, or layout changes
	 * 
	 * <p>
	 ...
	 * script code. Each layout corresponds to a single line of code. For example, the first
	 ...
	 * the layouts render contiguous lines of code. The recommended strategy for painting is thus:
	 * 
	 * <ol>
	 ...
	 * 
	 * @param program the program for the current script
	 * @param model the line/token model
	 * @param pixmap a map from pixels' y coordinates to layout index, i.e, line number
	 */
	void setProgram(Program program, EvtLayoutModel model, LayoutPixelIndexMap pixmap);
	 ...
	 * 
	 * <p>
	 * This is called at least once when the provider is added. It subsequently called whenever an
     * evt option changes. To receive other options, the provider will need to listen using its own
     * mechanism.
	 * 
	 * <p>
	 ...
	 */
	default void setOptions(EvtOptions options) {
	}
}
